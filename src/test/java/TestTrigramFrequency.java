 ...
 */
public class TestTrigramFrequency {
    Connection conn;
    Server server;
    @Before
    ...
    @Test
    public void testTrigramFrequency() throws Exception {
        OracleClient client = new OracleClient();
        WordFrequencyR resp =client.getTrigramFrequency("පාසල්", "පෙළ", "පොත්", 2012, "NEWS");
        assertEquals(resp.getFrequency(),1);
        resp =client.getTrigramFrequency("පාසල්", "පෙළ", "පොත්", "NEWS");
        assertEquals(resp.getFrequency(),1);
        resp =client.getTrigramFrequency("පාසල්", "පෙළ", "පොත්", 2012);
        assertEquals(resp.getFrequency(),1);
    }
}
