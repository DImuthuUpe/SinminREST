...
import com.sinmin.rest.beans.response.FrequentWordsAroundWordR;
 ...
 */
public class TestFrequentWordsAroundWord {
    Connection conn;
    Server server;
    @Before
    ...
    @Test
    public void testFrequentWordsAroundWord() throws Exception {
        OracleClient client = new OracleClient();
        FrequentWordsAroundWordR resp = client.getFrequentWordsAroundWord("මෙම", "NEWS", 2012, 3, 4);
        assertEquals(resp.getWords().length,4);
        resp = client.getFrequentWordsAroundWord("මෙම", 2012, 3, 4);
        assertEquals(resp.getWords().length,4);
        resp = client.getFrequentWordsAroundWord("මෙම", "NEWS", 3, 4);
        assertEquals(resp.getWords().length,4);
        resp = client.getFrequentWordsAroundWord("මෙම", 3, 4);
        assertEquals(resp.getWords().length,4);
    }
}
