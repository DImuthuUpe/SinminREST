 ...
 */
public class TestLatestArticlesForWord {
    Connection conn;
    Server server;
    @Before
    ...
    @Test
    public void testLatestArticlesForWord() throws Exception {
        OracleClient client = new OracleClient();
        ArticlesForWordR resp =client.getLatestArticlesForWord("මෙම", 2012, "NEWS", 3);
        assertEquals(resp.getArticles().length,4);

        resp =client.getLatestArticlesForWord("මෙම", "NEWS", 3);
        assertEquals(resp.getArticles().length,4);

        resp =client.getLatestArticlesForWord("මෙම", 2012, 3);
        assertEquals(resp.getArticles().length,4);

        resp =client.getLatestArticlesForWord("මෙම", 3);
        assertEquals(resp.getArticles().length,4);
    }
}
