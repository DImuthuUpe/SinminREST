...
public class TestTrigramCount {
    ...
    @Test
    public void testTrigramCount() throws Exception {
        OracleClient client = new OracleClient();
        WordCountR resp = client.getTrigramCount("NEWS", 2012);
        assertEquals(resp.getCount(),3899);
        resp = client.getTrigramCount( 2012);
        assertEquals(resp.getCount(),3899);
        resp = client.getTrigramCount("NEWS");
        assertEquals(resp.getCount(),3899);
        resp = client.getTrigramCount();
        assertEquals(resp.getCount(),3899);
    }
}
