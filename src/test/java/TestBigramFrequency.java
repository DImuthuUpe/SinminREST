 ...
 */
public class TestBigramFrequency {
    Connection conn;
    Server server;
    @Before
    ...
    @Test
    public void testBigramFrequency() throws Exception {
        OracleClient client = new OracleClient();
        WordFrequencyR resp =client.getBigramFrequency("සැබෑ", "කරන", 2012, "NEWS");
        assertEquals(resp.getFrequency(),1);
        resp =client.getBigramFrequency("සැබෑ", "කරන", "NEWS");
        assertEquals(resp.getFrequency(),1);
        resp =client.getBigramFrequency("සැබෑ", "කරන", 2012);
        assertEquals(resp.getFrequency(),1);
    }
}
