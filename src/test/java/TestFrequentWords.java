 ...
 */
public class TestFrequentWords {
    Connection conn;
    Server server;
    @Before
    ...
    public void testFrequentWords() throws Exception {
        ...
        FrequentWordR resp = client.getFrequentWords(2012, "NEWS", 3);
        assertEquals(resp.getValue1().length,3);

        resp = client.getFrequentWords(2012, 3);
        assertEquals(resp.getValue1().length,3);

        resp = client.getFrequentWords("NEWS", 3);
        assertEquals(resp.getValue1().length,3);

        resp = client.getFrequentWords(3);
        assertEquals(resp.getValue1().length,3);
    }
}
