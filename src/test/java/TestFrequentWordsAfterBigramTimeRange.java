...
import com.sinmin.rest.beans.response.FrequentWordsAfterWordR;
 ...
 */
public class TestFrequentWordsAfterBigramTimeRange {
    Connection conn;
    Server server;
    @Before
    ...
    @Test
    public void testFrequentWordsAfterBigramTimeRange() throws Exception {
        OracleClient client = new OracleClient();
        FrequentWordsAfterWordR resp =client.getFrequentWordsAfterBigramTimeRange("මිය", "ගිය", "NEWS", 2012, 2013, 2);
        assertEquals(resp.getWords().length,2);
        resp =client.getFrequentWordsAfterBigramTimeRange("මිය", "ගිය", 2012, 2013, 2);
        assertEquals(resp.getWords().length,2);
    }
}
