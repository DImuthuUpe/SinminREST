 ...
 */
public class TestFrequentWordsAfterWordTimeRange {
    Connection conn;
    Server server;
    @Before
    ...
    @Test
    public void testFrequentWordsAfterWordTimeRange() throws Exception {
        OracleClient client = new OracleClient();
        FrequentWordsAfterWordR resp= client.getFrequentWordsAfterWordTimeRange("මෙම", "NEWS", 2012, 2013, 4);
        assertEquals(resp.getWords().length,4);
        resp= client.getFrequentWordsAfterWordTimeRange("මෙම", 2012, 2013, 4);
        assertEquals(resp.getWords().length,4);
    }
}
